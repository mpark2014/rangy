xn.test.suite(
  ...
  function (s) {
    ...
    s.test("toggleRange simple test 2", function (t) {
      ...
      var testEl = document.getElementById("test");
      testEl.innerHTML =
        'Test <span id="one" class="test other">One</span> test';
      var oneEl = document.getElementById("one");
      var range = rangy.createRangyRange();
      range.selectNodeContents(oneEl);
      applier.toggleRange(range);
        ...
        if (node.nodeType == 1) {
          ...
          if (includeSelf) {
            ...
            for (var i = 0, len = node.attributes.length; i < len; ++i) {
              ...
              if (
                attr.specified &&
                !/^(id|href|class|style)$/.test(attr.name)
              ) {
                html +=
                  " " + attr.name + '="' + node.getAttribute(attr.name) + '"';
              }
            }
...
*/
    ...
    s.test("Test adding extra class with overlapping containers", function (t) {
      ...
      var applier2 = rangy.createClassApplier("c2", {
        elementProperties: { className: "extra" },
      });
