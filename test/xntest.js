...
(function () {
  ...
  var init = function () {
    ...
    progressBarContainer.className =
      "xn_test_progressbar_container xn_test_overallprogressbar_container";
    progressBar = progressBarContainer.appendChild(
    ...
    if (log4javascriptEnabled && !xn.test.disableLogging) {
      ...
      TestLogAppender.prototype.layout = new log4javascript.PatternLayout(
      ...
      if (xn.test.enableConsoleLogging) {
        log4javascript
          .getRootLogger()
          .addAppender(new log4javascript.BrowserConsoleAppender());
      }
    }
  ...
  Suite.prototype.updateProgressBar = function () {
    ...
    this.progressBar.style.width =
      "" + parseInt((100 * this._currentIndex) / this.tests.length) + "%";
  ...
  Suite.prototype.finish = function (timeTaken) {
    var newClass = this.testFailed
      ? "xn_test_suite_failure"
      : "xn_test_suite_success";
    var oldClass = this.testFailed
      ? "xn_test_suite_success"
      : "xn_test_suite_failure";
  ...
  Suite.prototype.runNextTest = function () {
    if (
      this._currentIndex == this.tests.length ||
      (anyFailed && xn.test.stopOnFail)
    ) {
      // finished!
  ...
  Test.prototype.reportFailure = function (name, timeTaken, msg, ex) {
    ...
    var text = document.createTextNode(
      this.name + " failed in " + timeTaken + "ms"
    );
    ...
    this.suite.reportContainer.appendChild(dd);
    if (ex && ex.stack) {
      var stackTraceContainer = this.suite.reportContainer.appendChild(
        document.createElement("code")
      );
      stackTraceContainer.className = "xn_test_stacktrace";
      stackTraceContainer.innerHTML = ex.stack
        .replace(/\r/g, "\n")
        .replace(/\n{1,2}/g, "<br />");
    }
    log(ex);
  ...
  Test.prototype.createLogReport = function () {
    if (this.logMessages.length > 0) {
      ...
      var logToggler = this.reportHeading.appendChild(
      ...
      // Create log report
      this.logReport = this.suite.reportContainer.appendChild(
      ...
      for (var i = 0, len = this.logMessages.length; i < len; i++) {
        logMessageDiv = this.logReport.appendChild(
          document.createElement("div")
        );
    ...
    if (!success) {
      ...
      while (regex.test(comment)) {
        comment = comment.replace(
          regex,
          displayStringForValue(values[parseInt(RegExp.$1)])
        );
      }
      this.fail(
        "Test failed on assertion " + this.assertCount + ": " + comment
      );
    }
  };
  ...
  Test.prototype.assertNotNull = function () {
    assert.apply(this, [
      ...
      1,
      testNotNull,
      "Expected not to be null but was {0}",
    ]);
  };
  ...
  Test.prototype.assert = function () {
    assert.apply(this, [
      ...
      1,
      testBoolean,
      "Expected not to be equivalent to false",
    ]);
  };
  ...
  Test.prototype.assertFalse = function () {
    assert.apply(this, [
      ...
      1,
      testFalse,
      "Expected to be false but was {0}",
    ]);
  };
  ...
  Test.prototype.assertEquivalent = function () {
    assert.apply(this, [
      ...
      2,
      testEquivalent,
      "Expected to be equivalent but values were {0} and {1}",
    ]);
  };
  ...
  Test.prototype.assertNotEquivalent = function () {
    assert.apply(this, [
      ...
      2,
      testNotEquivalent,
      "Expected to be not equal but values were {0} and {1}",
    ]);
  };
  ...
  Test.prototype.assertEquals = function () {
    assert.apply(this, [
      ...
      2,
      testEquals,
      "Expected to be equal but values were {0} and {1}",
    ]);
  };
  ...
  Test.prototype.assertRegexMatches = function () {
    assert.apply(this, [
      ...
      2,
      testRegexMatches,
      "Expected regex {0} to match value {1} but it didn't",
    ]);
  };
  ...
  Test.prototype.assertElementsEquivalent = function () {
    assert.apply(this, [
      ...
      "Expected elements {0} and {1} to have the same elements but they did not",
    ]);
  };
  ...
  Test.prototype.assertElementsNotEquivalent = function () {
    assert.apply(this, [
      arguments,
      2,
      testArraysHaveDifferentElements,
      "Expected elements {0} and {1} not to have the same elements but they did",
    ]);
  };
      ...
      if (errorType && !(e instanceof errorType)) {
        this.fail(
          "Expected error of type " +
            errorType +
            " to be thrown but error thrown was " +
            e
        );
      }
    }
  ...
  Test.prototype.assertArraysSameElements = function () {
    assert.apply(this, [
      arguments,
      2,
      testArraysSameElements,
      "Expected array {0} to have same set of elements as {1} but it didn't",
    ]);
  };
  ...
  Test.prototype.assertArraysNotSameElements = function () {
    assert.apply(this, [
      arguments,
      2,
      testArraysNotSameElements,
      "Expected array {0} to have a different set of elements to {1} but it didn't",
    ]);
  };

  var arrayElementsEquivalent =
    typeof Array.prototype.every == "function"
      ? function (array1, array2) {
          function check(val, idx) {
            return val === array2[idx];
  ...
  Test.prototype.assertArraysEquivalent = function () {
    assert.apply(this, [
      arguments,
      2,
      testArraysEquivalent,
      "Expected array {0} to have the same elements in the same order as {1} but it didn't",
    ]);
  };
  ...
  Test.prototype.assertArraysNotEquivalent = function () {
    assert.apply(this, [
      arguments,
      2,
      testArraysNotEquivalent,
      "Expected array {0} not to have the same elements in the same order as {1} but it did",
    ]);
  };
