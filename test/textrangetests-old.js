  ...
  function (s) {
    ...
    s.test("isCollapsedWhitespaceNode", function (t) {
      ...
      if (t.el.firstChild.lastChild) {
        t.assert(
          rangy.textRange.isCollapsedWhitespaceNode(t.el.firstChild.lastChild)
        );
      } else {
        // IE < 9 case
    ...
    */
    ...
    s.test("hasInnerText", function (t) {
      t.el.innerHTML =
        '<div></div><div> </div><div>1</div><div style="display: none">2</div><div class="xn_test_hidden">3</div>';
    ...
    s.test("innerText on simple text with trailing space", function (t) {
      t.el.innerHTML = "One Two ";
      var expectedText = rangy.features.trailingSpaceInBlockCollapses
        ? "One Two"
        : "One Two ";
    ...
    s.test(
      "innerText on simple text with trailing space in span",
      function (t) {
    ...
    s.test(
      "innerText on paragraph with uncollapsed br preceded by space",
      function (t) {
        t.el.innerHTML = "<p>1 <br>2</p>";
        t.assertEquals(rangy.innerText(t.el), "1\n2");
      }
