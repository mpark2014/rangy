...
*/
    ...
    function splitNodeAt(node, descendantNode, descendantOffset) {
      log.debug(
        ...
        dom.inspectNode(descendantNode),
        descendantOffset
      );
      var newNode;
      ...
      if (!newNode) {
        ...
        while ((child = descendantNode.childNodes[descendantOffset])) {
          log.debug(
            "Moving node " +
              dom.inspectNode(child) +
              " into " +
              dom.inspectNode(newNode)
          );
      ...
      return descendantNode == node
        ? newNode
        : splitNodeAt(node, newNode.parentNode, dom.getNodeIndex(newNode));
    }
    ...
    TextCommand.prototype = {
        ...
        for (var i = 0, len = textNodes.length; i < len; ++i) {
          ...
          log.debug(
            "Checking for merge. text node: " +
              textNode.data +
              ", preceding: " +
              (precedingTextNode ? precedingTextNode.data : null)
          );
      ...
      isRemovable: function (el) {
        return (
          ...
          trim(el.className) == this.className &&
          !elementHasNonClassAttributes(el)
        );
      },
      ...
      undoToTextNode: function (textNode, range, appliedAncestor) {
        log.info(
          ...
          range.inspect(),
          dom.inspectNode(appliedAncestor),
          range.containsNode(appliedAncestor)
        );
        ...
        log.info(
          "isRemovable",
          this.isRemovable(appliedAncestor),
          dom.inspectNode(appliedAncestor),
          appliedAncestor.innerHTML,
          appliedAncestor.parentNode.innerHTML
        );
        ...
        for (var i = 0, len = textNodes.length, selectedText; i < len; ++i) {
          ...
          log.debug(
            "text node: '" +
              ...
              selectedText +
              "'",
            this.isAppliedToElement(textNodes[i].parentNode)
          );
          if (
            selectedText != "" &&
            !this.isAppliedToElement(textNodes[i].parentNode)
