...
rangy.createModule("Position", ["WrappedSelection"], function (api, module) {
  ...
  function getScrollPosition(win) {
    ...
    if (typeof win.pageXOffset == NUMBER && typeof win.pageYOffset == NUMBER) {
      ...
      var scrollEl =
        typeof compatMode == "string" && compatMode.indexOf("CSS") >= 0 && docEl
          ? docEl
          : dom.getBody(doc);
  ...
  function adjustClientRect(rect, doc) {
    ...
    var container =
      docEl.clientWidth === 0 && typeof body.clientTop == NUMBER ? body : docEl;
    ...
    if (!node) {
      ...
      main: while (node) {
        ...
        if (node.nodeType == 3) {
          ...
          for (
            var offset = 0, textLen = node.length;
            offset <= textLen;
            ++offset
          ) {
  ...
  var elementSupportsGetBoundingClientRect = util.isHostMethod(
    span,
  ...
  if (
    ...
    rangeProto.getBoundingClientRect = function () {
      ...
      if (cells.length > 0) {
        ...
        for (
          var i = 0, cell, tempTextRange, table, subRange, subRect;
          (cell = cells[i]);
          ++i
        ) {
        ...
        if (!endTable && lastTable) {
          ...
          subRange.setStartAfter(lastTable);
          rects.push(
            WrappedTextRange.rangeToTextRange(subRange).getBoundingClientRect()
          );
        }
