...
*/
  ...
  function isContained(node, range) {
    return (
      ...
      dom.comparePoints(node, 0, range.startContainer, range.startOffset) ==
        1 &&
      dom.comparePoints(
        node,
  ...
  function isEffectivelyContained(node, range) {
    ...
    if (childCount != 0) {
      ...
      if (
        (!dom.isAncestorOf(node, sc) || sc.nodeType != 3 || so == 0) &&
        (!dom.isAncestorOf(node, ec) ||
          ec.nodeType != 3 ||
          eo == dom.getNodeLength(ec))
      ) {
        return true;
      }
    }
  ...
  function isCollapsedWhitespaceNode(node) {
    ...
    // "none", return true."
    if (
      hasAncestor(node, function (ancestor) {
        return (
          ancestor.nodeType == 1 &&
          getComputedStyleProperty(ancestor, "display") == "none"
        );
      })
    ) {
      return true;
    ...
    while (reference != ancestor) {
      ...
      if (
        (reference.nodeType == 3 && !isWhitespaceNode(reference)) ||
  ...
  function isVisible(node) {
    ...
    if (
      isOrHasAncestor(node, function (node) {
        return (
          node.nodeType == 1 &&
          getComputedStyleProperty(node, "display") == "none"
        );
      })
    ) {
      return false;
  ...
  function getEffectiveTextNodes(range, context) {
    return range.getNodes([3], function (node) {
      return (
        ...
        (!context.options.ignoreInvisibleNodes || isInvisible(node))
      );
    });
  }
...
*/
  ...
  function clearValue(element, context) {
    ...
    log.info(
      "clearValue",
      outerHtml(element),
      command.getSpecifiedValue(element, context)
    );
  ...
  var defaultSiblingCriteria = function () {
    return false;
  };
  var defaultNewParentInstructions = function () {
    return null;
  };
  ...
  function wrap(nodeList, context, siblingCriteria, newParentInstructions) {
    ...
    newParentInstructions =
      newParentInstructions || defaultNewParentInstructions;
    ...
    if (!newParentParent) {
      ...
      for (var i = 0; (range = rangesToPreserve[i++]); ) {
        if (
          ...
          range.startOffset == newParentNodeIndex
        ) {
          range.setStart(range.startContainer, range.startOffset + 1);
        }
        if (
          range.endContainer == newParentParent &&
          range.endOffset == newParentNodeIndex
        ) {
