...
rangy.createModule("Commands", ["WrappedSelection"], function (api, module) {
  ...
  function isBefore(nodeA, nodeB) {
    var parentA = nodeA.parentNode,
      parentB = nodeB.parentNode;
    if (parentA && parentB) {
      return (
        dom.comparePoints(
          nodeA.parentNode,
          dom.getNodeIndex(nodeA),
          nodeB.parentNode,
          dom.getNodeIndex(nodeB)
        ) == -1
      );
    } else {
      return !parentA;
  ...
  function isContained(node, range) {
    var pos1 = dom.comparePoints(
      node,
      0,
      range.startContainer,
      range.startOffset
    );
    var pos2 = dom.comparePoints(
      node,
    ...
    return (
      getRootContainer(node) == getRootContainer(range.startContainer) &&
      pos1 == 1 &&
      pos2 == -1
    );
  }
  ...
  function isEffectivelyContained(node, range) {
    ...
    if (
      ...
      isCharData &&
      dom.getNodeLength(node) != range.startOffset
    ) {
      return true;
    }
  ...
  function isHtmlNode(node) {
    var ns;
    return (
      typeof (ns = node.namespaceURI) == UNDEF ||
      ns === null ||
      ns == "http://www.w3.org/1999/xhtml"
    );
  }

  var unwrappableTagNamesRegex =
    /^(h[1-6]|p|hr|pre|blockquote|ol|ul|li|dl|dt|dd|div|table|caption|colgroup|col|tbody|thead|tfoot|tr|th|td|address)$/i;
  ...
  function isHtmlElement(node, tagNames) {
    ...
    switch (typeof tagNames) {
      ...
      case "object":
        return new RegExp("^(" + tagNames.join(",") + ")$", "i").test(
          node.tagName
        );
      default:
        return true;
...
*/
  ...
  function isIgnoredNode(node, options) {
    ...
    if (node.nodeType == 8) {
      ...
      if (
        node.parentNode &&
        /^(script|style)$/i.test(node.parentNode.tagName)
      ) {
      ...
      if (
        ...
        !/[^\r\n\t ]/.test(node.data) &&
        (isUnwrappable(node.previousSibling, options) ||
          isUnwrappable(node.nextSibling, options))
      ) {
        return true;
      }
    }
  ...
  function blockExtend(range) {
    ...
    while (true) {
      ...
      if (dom.isCharacterDataNode(startNode) || startOffset == 0) {
        ...
        (startChildNode = startNode.childNodes[startOffset]) &&
    ...
    while (true) {
      ...
      if (endOffset == 0) {
        ...
        dom.isCharacterDataNode(endNode) ||
        ...
        (endChildNode = endNode.childNodes[endOffset]) &&
  ...
  function elementOnlyHasAttributes(el, attrs) {
    ...
    for (var i = 0, len = el.attributes.length, attrName; i < len; ++i) {
      ...
      if (
        ...
        (!attrs || !dom.arrayContains(attrs, attrName))
      ) {
        return false;
      }
    }
  ...
  var simpleModifiableElementRegex = new RegExp(
    "^(" + simpleModifiableElements + ")$"
  );
  ...
  function isSimpleModifiableElement(el, context) {
    ...
    if (
      attrName != "style" ||
      (typeof el.style.length == "number" && el.style.length > 1) ||
    ...
    // "text-decoration"."
    if (
      (tagName == "a" || tagName == "font" || tagName == "span") &&
    ...
    // "overline" or "none"."
    if (
      /^(a|font|s|span|strike|u)$/.test(tagName) &&
      /^(line-through|underline|overline|none)$/.test(el.style.textDecoration)
    ) {
      return true;
    }
  ...
  function movePreservingRanges(node, newParent, newIndex, rangesToPreserve) {
    ...
    for (var j = 0, rangeMove; (rangeMove = rangeMoves[j++]); ) {
      log.debug(
        "Moving " + rangeMove[0].inspect(),
        dom.inspectNode(rangeMove[1]),
        rangeMove[2],
        dom.inspectNode(rangeMove[3]),
        rangeMove[4]
      );
  ...
  function decomposeRange(range, rangesToPreserve) {
    ...
    while (
      ...
      clonedRange.setStart(
        clonedRange.startContainer.parentNode,
        dom.getNodeIndex(clonedRange.startContainer)
      );
    }
    ...
    while (
      clonedRange.endOffset == dom.getNodeLength(clonedRange.endContainer) &&
      clonedRange.endContainer.parentNode
    ) {
      clonedRange.setEnd(
        clonedRange.endContainer.parentNode,
        1 + dom.getNodeIndex(clonedRange.endContainer)
      );
    }
  ...
  function moveChildrenPreservingRanges(
    node,
    newParent,
    newIndex,
    removeNode,
  ...
  function replaceWithOwnChildren(element, rangesToPreserve) {
    return moveChildrenPreservingRanges(
      element,
      element.parentNode,
  ...
  function clearValue(element, context) {
    var command = context.command,
      rangesToPreserve = context.rangesToPreserve;
  ...
  function removeExtraneousLineBreaksBefore(node) {
    ...
    if (
      isHtmlElement(previousSibling, "br") &&
      isInlineNode(
        (previousSiblingPreviousSibling = node.previousSibling.previousSibling)
      ) &&
  ...
  function removeExtraneousLineBreaksAtTheEndOf(node) {
    ...
    if (
      ...
      isInlineNode(
        (lastChildPreviousSibling = node.lastChild.previousSibling)
      ) &&
  ...
  function wrap(nodeList, siblingCriteria, newParentInstructions, context) {
    ...
    if (
      isInlineNode(lastNode) &&
      !isHtmlElement(lastNode, "br") &&
      isHtmlElement(lastNode.nextSibling, "br")
    ) {
    ...
    var newParent,
      nodePriorToFirstNode = firstNode.previousSibling,
      nodeAfterLastNode = lastNode.nextSibling;

    if (
      isEditable(nodePriorToFirstNode, options) &&
      siblingCriteria(nodePriorToFirstNode)
    ) {
      ...
      isEditable(nodeAfterLastNode, options) &&
      siblingCriteria(nodeAfterLastNode)
    ) {
    ...
    var newParentParent = newParent.parentNode,
      firstNodeParent = firstNode.parentNode;
    ...
    if (!newParentParent) {
      ...
      for (var i = 0; (range = rangesToPreserve[i++]); ) {
        if (
          range.startContainer == newParentParent &&
          range.startOffset == dom.getNodeIndex(newParent)
        ) {
          range.setStart(range.startContainer, range.startOffset + 1);
        }
        if (
          range.endContainer == newParentParent &&
          range.endOffset == dom.getNodeIndex(newParent)
        ) {
    ...
    if (isBefore(newParent, firstNode)) {
      ...
      // child of new parent."
      if (
        !isInlineNode(newParent) &&
        isInlineNode(newParent.lastChild) &&
        isInlineNode(firstNode) &&
    ...
    // parent."
    if (
      isEditable(firstNodeParent, options) &&
      !firstNodeParent.hasChildNodes()
    ) {
    ...
    if (
      isEditable(newParentNextSibling, options) &&
      siblingCriteria(newParentNextSibling)
    ) {
      ...
      if (
        !isInlineNode(newParent) &&
        isInlineNode(newParent.lastChild) &&
        isInlineNode(newParentNextSibling.firstChild) &&
        !isHtmlElement(newParent.lastChild, "br")
      ) {
        newParent.appendChild(doc.createElement("br"));
      }
      ...
      while (newParentNextSibling.hasChildNodes()) {
        movePreservingRanges(
          newParentNextSibling.firstChild,
          newParent,
          -1,
          rangesToPreserve
        );
  ...
  function reorderModifiableDescendants(
    node,
    command,
    newValue,
    context,
  ...
  function forceValue(node, newValue, context) {
    ...
    if (/^(1|3|4|7)$/.test("" + nodeType) && !isUnwrappable(node, options)) {
      ...
      wrap(
        ...
        function (node) {
          return (
            isSimpleModifiableElement(node, context) &&
            valuesEqual(
              command,
              command.getSpecifiedValue(node, context),
              newValue
            ) &&
    ...
    if (isUnwrappable(node, options)) {
      ...
      children = [];
      for (
        var i = 0, len = node.childNodes.length, specifiedValue;
        i < len;
        ++i
      ) {
    ...
    if (
      property !== null &&
      !valuesEqual(command, getEffectiveValue(newParent, context), newValue)
    ) {
    ...
    if (
      ...
      if (
        newParent.tagName.toLowerCase() == "span" &&
        ((command.hasSpecialSpanStyling &&
          command.hasSpecialSpanStyling(newValue)) ||
          property !== null)
      ) {
  ...
  function pushDownValues(node, context) {
    ...
    while (
      isEditable(currentAncestor, options) &&
      currentAncestor.nodeType == 1 &&
      !valuesEqual(
        command,
        getEffectiveValue(currentAncestor, context),
        newValue
      )
    ) {
    ...
    if (
      ...
      !valuesEqual(
        command,
        getEffectiveValue(lastAncestor.parentNode, context),
        newValue
      )
    ) {
      return;
      ...
      for (var i = 0, child; (child = children[i++]); ) {
        ...
        if (
          ...
          !valuesEqual(
            command,
            propagatedValue,
            command.getSpecifiedValue(child, context)
          )
        ) {
          continue;
        }
  ...
  function getEffectiveTextNodes(range, context) {
    return range.getNodes([3], function (node) {
      return (
        ...
        !isIgnoredNode(node, context.options)
      );
    });
  }
  ...
  function getCommand(name) {
    ...
    if (commandsByName.hasOwnProperty(lowerName)) {
      ...
      throw module.createError(
        "getCommand(): No command registered with the name '" + name + "'"
      );
    }
  }
  ...
  api.queryCommandValue = function (name, options, range) {
    options = options || {};
    var win = options.hasOwnProperty("document")
      ? dom.getWindow(options.document)
      : window;
    var value = options.hasOwnProperty("value") ? options.value : null;
    var command = getCommand(name);
    var sel = api.getSelection(win);
